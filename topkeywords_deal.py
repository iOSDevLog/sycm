...
import os
import datetime
from searchDeal import *
import csv

def fetchDealTopKeywords(date, rival1Id, rival2Id):
    dateRange = date + "|" + date
    refererUrl = "https://sycm.taobao.com/ci/monitor/itemcompare"

    TOP_TYPE_STREAM = 1
    TOP_TYPE_DEAL = 2
    topType = TOP_TYPE_DEAL
    ...
    # with open(date + '_' + str(rival1Id) + '_deal_key_words.csv', 'w+', newline='') as csvfile:
    #     for rival in rival1:
    #         print(rival.keyword.value + "," + str(rival.pay_item_cnt.value))

    #         writer = csv.writer(csvfile)
    #         writer.writerow([rival.keyword.value , rival.pay_item_cnt.value])

    # # print('\n\n')
    # # writer.writerow([rival.keyword.value , rival.uv.value])

    # with open(date + '_' + str(rival2Id) + '_deal_key_words.csv', 'w+', newline='') as csvfile:
    #     for rival in rival2:
    #         print(rival.keyword.value + "," + str(rival.pay_item_cnt.value))
